...
describe('Order Filter Schema Tests', () => {
    ...
    describe('Sorting Props Tests', () => {
        ...
        it('should validate valid when sorting props are null', () => {
            const result = OrderFilterSchema.safeParse({
                sortBy: null,
                sortDirection: null,
                currentPage: 1,
    ...
    describe('Pagination Props Tests', () => {
        ...
        it('should validate invalid when filtering with too low current page value', () => {
            ...
            expect(result.success).toBe(false);
            expect(result.error.issues[0].message).toBe('Current page must be greater than 0');
        });
        it('should validate invalid when you filtering with too low limit value', () => {
            ...
            expect(result.success).toBe(false);
            expect(result.error.issues[0].message).toBe('Pagination limit must be greater than 5');
        })
        it('should validate invalid when you filtering with loo hight limit value', () => {
            const result = OrderFilterSchema.safeParse({
                currentPage: 1,
                limit: 21,
            })
            expect(result.success).toBe(false);
            expect(result.error.issues[0].message).toBe('Pagination limit must be less than 20');
        })
    })
})