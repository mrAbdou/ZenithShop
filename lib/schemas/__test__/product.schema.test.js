...
describe('Product Filter Schema Test', () => {
    ...
    it('should validate invalid when limit takes a wrong value', () => {
        ...
        expect(result.success).toBe(false)
        expect(result.error.issues[0].message).toBe('Limit should be at most 50')
    });
    ...
    it('should validate invalid when filters has only contains sorting props', () => {
        const result = ProductPaginationSchema.safeParse({
            sortBy: 'name',
            sortDirection: 'asc',
        });
    ...
    it('should validate invalid when filters is empty', () => {
        const result = ProductPaginationSchema.safeParse({});
        expect(result.success).toBe(false);
        expect(result.error.issues[0].message).toBe('Limit should be a number');
        expect(result.error.issues[1].message).toBe('Current page should be a number');
