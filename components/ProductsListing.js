'use client';
import { LIMIT } from "@/lib/tanStackHooks/constants";
import { useInfiniteQuery } from "@tanstack/react-query";
import Product from "./Product";
import { useProducts } from "@/lib/tanStackHooks/products";
export default function ProductsListing({ limit = LIMIT, offset = 0 } = {}) {
    const { data, isLoading, hasNextPage, isFetchingNextPage, fetchNextPage } = useInfiniteQuery({
        queryKey: ["products", limit, offset],
        queryFn: ({ pageParam = offset }) => useProducts(limit, offset = pageParam),
